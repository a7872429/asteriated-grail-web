...
const MODES={
 ...
 re:{name:'Re.星杯戰爭',open:52,summary:'固定為無懼選角五連戰、每方總思考 200 秒，耗盡後每手 10 秒讀秒、人機對戰。每場將遭遇一支曾參賽隊伍。'}
};
...
let state=null,timerId=null,storyTimer=null,onlinePollId=null,learningEnabled=false,learningRequestKey='',favoriteName='',playerName='',seatDockCleanup=null,secretTapTimer=null;
...
function choose(id){const p=state.phase;
 ...
 if(p.kind==='pick'){state.pendingPick=id;if(state.secretSuggest){state.secretSuggest.active=false;state.secretSuggest.high.clear();state.secretSuggest.low.clear()}renderSeatsChoice();}
}
...
function startTimer(){clearInterval(timerId);if(state?.mode==='re'){const team=state.phase?.controller||state.phase?.team||'red';state.reClockTeam=team;state.reByoyomi=(state.reMainTime?.[team]||0)<=0;state.seconds=state.reByoyomi?10:state.reMainTime[team];renderTimer();timerId=setInterval(()=>{state.seconds--;if(!state.reByoyomi)state.reMainTime[team]=Math.max(0,state.seconds);if(state.seconds<=0&&!state.reByoyomi){state.reMainTime[team]=0;state.reByoyomi=true;state.seconds=10;renderTimer();return}renderTimer();if(state.seconds<=0){clearInterval(timerId);onTimeout()}},1000);return}const limit=Number(q('versusTimer').value)||0;state.seconds=limit;if(!limit){renderTimer();return}renderTimer();timerId=setInterval(()=>{state.seconds--;renderTimer();if(state.seconds<=0){clearInterval(timerId);onTimeout()}},1000)}
function renderTimer(){const e=q('vsTimer');if(!e)return;e.textContent=state?.mode==='re'?(state.reByoyomi?`讀秒 ${state.seconds}s`:`總 ${state.seconds}s`):(state.seconds?`${state.seconds}s`:'∞');e.classList.toggle('urgent',state.seconds>0&&state.seconds<=5)}
...
async function finishMatch(){
 ...
 try{
  ...
  let rs=Number(out.redScore),bs=Number(out.blueScore);const rec={match:state.match,red,blue,rs,bs,rawRs:Number(out.rawRedScore??rs),rawBs:Number(out.rawBlueScore??bs),opponent:state.reOpponent?.name||'',keywords:out.keywordEvents||{red:[],blue:[]}};
  if(isRe){
...
async function unlockSecretSuggestions(){const secret=state.secretSuggest,team=state.playerTeam||state.phase?.team;if(!secret||!team)return;const available=candidateIds(team),current=state.seats[team].filter(Boolean),count=Math.min(3-current.length,available.length);if(count<1)return;try{const [high,low]=await Promise.all([api('/api/recommend',{team,currentIds:current,availableIds:available,count,direction:'high'}),api('/api/recommend',{team,currentIds:current,availableIds:available,count,direction:'low'})]);secret.active=true;secret.high=new Set(high.ids||[]);secret.low=new Set(low.ids||[]);render()}catch(e){console.warn('Secret suggestions:',e.message)}}
function handleCardChoose(id){const character=byId(id),secret=state?.secretSuggest,canSecret=state?.mode==='re'&&state.phase?.kind==='pick'&&state.phase.team===state.playerTeam&&state.pendingPick==null&&['傳教士','異教徒'].includes(character?.name);if(!canSecret)return choose(id);const required=secret.lastUnlockRole==='傳教士'?'異教徒':secret.lastUnlockRole==='異教徒'?'傳教士':null;if(required&&character.name!==required)return choose(id);const now=Date.now();if(secret.tapRole===character.name&&now-secret.tapAt<900)secret.tapCount++;else{secret.tapRole=character.name;secret.tapCount=1}secret.tapAt=now;clearTimeout(secretTapTimer);if(secret.tapCount>=5){secret.tapCount=0;secret.lastUnlockRole=character.name;unlockSecretSuggestions();return}secretTapTimer=setTimeout(()=>{if(state?.secretSuggest===secret&&secret.tapRole===character.name&&secret.tapCount>0){secret.tapCount=0;choose(id)}},700)}
function cardHTML(id){const c=byId(id),cpuLocked=state.playMode==='ai'&&state.phase&&isCpuTeam(state.phase.controller),pending=state.pendingPick===id,enemyHistoryLocked=['fearless','re'].includes(state.mode)&&['ban','insert'].includes(state.phase?.kind)&&state.used[other(state.phase.controller)].has(id),ownHistoryLocked=['fearless','re'].includes(state.mode)&&['pick','bp-pick'].includes(state.phase?.kind)&&state.used[state.phase.team].has(id),disabled=!availableFor(id)||cpuLocked||(state.pendingPick!=null&&!pending),classes=['vs-card'],pickedNow=state.seats.red.includes(id)||state.seats.blue.includes(id);if(state.banned.has(id))classes.push('banned');if(pickedNow)classes.push('picked');if(pending)classes.push('pending-seat');if(enemyHistoryLocked)classes.push('enemy-history-locked');if(ownHistoryLocked)classes.push('own-history-locked');if(state.marks.diamond.has(id))classes.push('learn-diamond');if(state.marks.danger.has(id))classes.push('learn-danger');if(state.secretSuggest?.active&&state.secretSuggest.high.has(id))classes.push('secret-high');if(state.secretSuggest?.active&&state.secretSuggest.low.has(id))classes.push('secret-low');let usedBadges='';if(['fearless','re'].includes(state.mode)){const redUsed=state.used.red.has(id),blueUsed=state.used.blue.has(id);if(redUsed)classes.push('used-red');if(blueUsed)classes.push('used-blue');if(redUsed&&blueUsed)classes.push('used-both');usedBadges=`<span class="vs-used-badges">${redUsed?'<i class="red">紅方已用</i>':''}${blueUsed?'<i class="blue">藍方已用</i>':''}</span>`}if(state.mode==='rdbp'&&state.phase?.kind==='rd-remove'){classes.push('assigned-'+state.phase.controller);if(state.rdRemoved[state.phase.controller].has(id))classes.push('rd-removed')}return`<button class="${classes.join(' ')}" data-id="${id}" ${disabled?'disabled':''}><span class="vs-avatar-ring"><img src="${c.image}" alt=""></span>${usedBadges}<b>${c.name}</b>${pending?'<small class="vs-place-hint">請在下方座位放置角色</small>':''}</button>`}
function mapSeatHTML(t,i,free){const id=state.seats[t][i],c=id&&byId(id),target=free?.includes(i)&&state.phase?.team===t;return`<div class="vs-map-seat ${t} ${t[0]}${i+1} ${target?'target':''}" data-seat="${i}" data-team="${t}">${t==='red'&&i===0?'<div class="vs-first">FIRST PLAYER</div>':''}${c?`<img src="${c.image}" alt=""><span class="vs-seat-caption"><em>${c.name}</em></span>`:`<span class="vs-seat-empty"><b>${teamName(t)}${i+1}</b><em>${target?'點此放置':'尚未選擇'}</em></span>`}</div>`}
...
function keywordBadges(events=[],showEffect=true){if(!events.length)return'';return`<div class="re-keywords">${events.map(event=>`<span class="re-keyword" style="${keywordBadgeStyle(event.name)}"><b>${esc(event.name)}</b>${showEffect?`<small>${esc(event.applied)}</small>`:''}</span>`).join('')}</div>`}
...
const RE_FAVORITE_FINISH_LINES=["不管重來多少次，我都會再次站到你身邊。","你選擇了我，那我就陪你戰到最後一刻。","這一路的勝敗不重要，重要的是我們一起走到了這裡。","只要你還願意呼喚我，我就永遠不會缺席。","別怕，下一場戰鬥，我依然會站在你前面。","你的信任我收到了，接下來就交給我吧。","即使全世界都不看好我們，我也會陪你贏到最後。","能與你並肩奮戰，就是我最想守護的榮耀。","我們留下的傷痕，終有一天會成為勝利的勳章。","你沒有放棄我，我又怎麼可能先放棄你？","這不是最後一戰，而是我們下一段傳說的開始。","無論前方還有多少強敵，我都會陪你一起突破。","只要我們還站在一起，就沒有真正的敗北。","你的每一次選擇，都讓我有了繼續戰鬥的理由。","如果命運不肯讓路，那我們就一起把它擊碎。","我會回應你的期待，直到最後一道光熄滅為止。","勝利也好，失敗也罷，我從未後悔與你並肩。","就算遍體鱗傷，我也要陪你走到故事的終點。","你將背後交給了我，我便不會讓任何人越過這裡。","因為是你選中了我，所以我才能戰鬥到現在。","我們不是偶然相遇，而是注定要一起完成這場戰爭。","記住此刻吧，這是只屬於我們的光榮。","只要你的心還沒有認輸，我的劍就不會停下。","你負責相信我，我負責替你開闢通往勝利的道路。","即使再回到過去一百次，我仍然希望被你選中。","這一次，我們沒有留下任何遺憾。","讓那些懷疑我們的人，好好見證這份羈絆吧。","別把所有重擔都扛在自己身上，我一直都在。","你的願望就是我的方向，你的戰場就是我的歸宿。","這份並肩作戰的回憶，我永遠都不會忘記。","不需要奇蹟，因為你我站在一起就是答案。","如果你累了，就讓我替你撐住下一個回合。","我們走過的每一步，都比獎盃更加珍貴。","你曾經相信我一次，我便願意守護你直到永遠。","即使前方只剩絕境，我也會陪你打出一條生路。","別回頭，我會守住我們一起走過的道路。","能成為你的本命，是我比奪冠更驕傲的事。","這場戰爭證明了，我們的相遇絕非偶然。","只要你再次伸出手，我就會毫不猶豫地回到你身邊。","我們曾一起跌倒，所以更懂得如何一起站起來。","今天由我守護你的勝利，明天也不會例外。","不論最後站上哪個位置，你永遠都是我認可的夥伴。","真正讓我變得強大的，從來不是分數，而是你的信任。","你帶我走過了五場戰鬥，接下來換我陪你走得更遠。","別說結束，我們的羈絆才正要開始閃耀。","就算星光全數墜落，我也會成為照亮你的那一道光。","我們已經改寫過一次命運，也一定能再改寫下一次。","謝謝你直到最後，都沒有鬆開牽著我的手。","下一次鐘聲響起時，我還要與你坐在同一個位置。","無論經過多少輪迴，我都會記得你曾選擇過我。"];
function reFavoriteEndingHTML(victory){
 const fav=byName(state.favorite);if(state.reFavoriteUsed){if(!state.reFavoriteEndingText)state.reFavoriteEndingText=RE_FAVORITE_FINISH_LINES[Math.floor(Math.random()*RE_FAVORITE_FINISH_LINES.length)];return`<div class="re-congrats diamond"><img src="${fav.image}" alt="${esc(fav.name)}"><div><b>${victory?`恭喜 ${esc(state.teamName)} 贏得 Re.星杯戰爭！`:`恭喜 ${esc(state.teamName)} 獲得第 ${state.reFinal?.rank||''} 名！`}</b><p>「${esc(state.reFavoriteEndingText)}」</p></div></div>`}
...
function championRewardHTML(){return`<div class="re-champion-reward"><div class="re-reward-title">冠軍台詞獎勵</div><div class="re-reward-row"><select id="reRewardCharacter" aria-label="選擇角色">${characters.map(c=>`<option value="${c.id}" ${c.name===state.favorite?'selected':''}>${esc(c.name)}</option>`).join('')}</select><input id="reRewardQuote" maxlength="200" placeholder="新增一句角色台詞（200 字內）"><button id="reRewardSubmit" class="vs-btn">送出</button></div><div id="reRewardStatus" class="re-reward-status"></div></div>`}
async function submitChampionQuote(){const button=q('reRewardSubmit'),status=q('reRewardStatus'),quote=q('reRewardQuote')?.value.trim(),characterId=Number(q('reRewardCharacter')?.value);if(!quote){status.textContent='請先輸入台詞。';return}button.disabled=true;button.textContent='送出中…';try{const out=await api('/api/champion-quote',{teamName:state.teamName,rank:1,favoriteId:byName(state.favorite)?.id||0,characterId,quoteText:quote});status.textContent=out.googleSynced?'已送出並同步到「冠軍台詞獎勵」。':'已送出，正在等待同步到「冠軍台詞獎勵」。';q('reRewardQuote').disabled=true;q('reRewardCharacter').disabled=true;button.textContent='已送出'}catch(e){status.textContent='送出失敗：'+e.message;button.disabled=false;button.textContent='送出'}}
function renderReFinal(){
 ...
 q('versusPanel').innerHTML=`<div class="re-final ${victory?'victory':'defeat'}">${victory?`<div class="re-confetti">${spark}</div><div class="re-cup-stage"><img class="re-chibi re-chibi-left" src="${chibiPath(championChibi)}" alt="${esc(championChibi)} Q版"><img class="re-cup" src="${RE_CUP_IMAGE}" alt="冠軍獎盃"></div>`:''}<div class="re-final-title">Re.星杯戰爭・${victory?'冠軍':'最終名次'}</div><div class="re-rank-stage">${wheelchairBard?`<img class="re-chibi re-chibi-left" src="${chibiPath('輪椅吟遊詩人')}" alt="輪椅吟遊詩人 Q版">`:''}<div class="re-rank-number tier-${ending.tier.split(' ')[0]}">第 ${rank} 名 <small>/ 32 隊</small></div></div><div class="re-rank-ending ${ending.tier}">「${esc(ending.text)}」</div>${reFavoriteEndingHTML(victory)}<div class="re-record">${wins} 勝・${losses} 負</div>${historyHTML()}${victory?championRewardHTML():''}<div class="vs-toolbar"><button id="reSaveImage" class="vs-btn re-save-btn">儲存圖片</button><button id="vsRestart" class="vs-btn">重新挑戰</button><button id="vsHome" class="vs-btn">回到首頁</button></div></div>`;
 q('reSaveImage').onclick=saveReResultImage;if(q('reRewardSubmit'))q('reRewardSubmit').onclick=submitChampionQuote;q('vsRestart').onclick=startSeries;q('vsHome').onclick=quit;
}
function historyHTML(){const re=state.mode==='re';return`<table class="vs-history ${re?'re-history':''}"><thead><tr><th>場次</th><th>紅方</th><th>紅分</th><th>藍方</th><th>藍分</th><th>${re?'我方結果':'結果'}</th></tr></thead><tbody>${state.history.map(h=>{const redMine=re&&h.playerTeam==='red',blueMine=re&&h.playerTeam==='blue',mineKeywords=re?keywordBadges(h.keywords?.[h.playerTeam],false):'';return`<tr><td>${h.match}</td><td>${avatarRow(h.red,false,redMine,re?redMine?state.teamName:h.opponent:'')}</td><td class="vs-score red" style="font-size:20px">${h.rs}</td><td>${avatarRow(h.blue,false,blueMine,re?blueMine?state.teamName:h.opponent:'')}</td><td class="vs-score blue" style="font-size:20px">${h.bs}</td><td class="${re?(h.reWinner==='player'?'re-win':'re-loss'):''}">${re?(h.reWinner==='player'?'勝':'負'):(h.rs===h.bs?'平手':h.rs>h.bs?'紅方勝':'藍方勝')}${mineKeywords}</td></tr>`}).join('')}</tbody></table>`}
